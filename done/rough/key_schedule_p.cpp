#include <iostream>

using namespace std;

void rot_word(unsigned char round_key[4][44], unsigned char rot_word[4])
{
	unsigned char temp;
		
	temp = round_key[0][3];
	
	rot_word[0] = round_key[1][3];
	rot_word[1] = round_key[2][3];
	rot_word[2] = round_key[3][3];
	rot_word[3] = temp;
}

void s_box(unsigned char state[4])
{
	const unsigned char s[16][16] = 
	{
	   
	};
	
		
	for(int i = 0; i < 4; i++)
	{
		unsigned char n = state[i];
		char t = 0xF & n;
		n = n >> 4;
			
		state[i] = s[n][t];										//N then T as it goes rows then columns
	}
}


int main()
{
	unsigned char cipher_key[4][4] = {
										{0x2b, 0x28, 0xab, 0x09},
										{0x7e, 0xae, 0xf7, 0xcf},
										{0x15, 0xd2, 0x15, 0x4f},
										{0x16, 0xa6, 0x88, 0x3c}
									};
									
	unsigned char round_key[4][44];
	
	unsigned char arot_word[4];
	
	unsigned char temp[4];
	
	
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 4; j++)
		{
			round_key[i][j] = cipher_key[i][j];
		}
	}
	
									
	unsigned char r_con[4][10] = {
									{0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x30, 0x80, 0x1b, 0x36},
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
									{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
								};
								
	
	for(int i = 4; i < 44; i++)
	{	
		
		if(i % 4 == 0)
		{
			
			rot_word(round_key, arot_word);
			s_box(arot_word);
			
			for(int j = 0; j < 4; j++)
			{
				temp[j] = ( round_key[j][i-4] ^ arot_word[j] ^ r_con[j][(i/4)-1] );
			}
			
			for(int j = 0; j < 4; j++)
			{
				round_key[j][i] = temp[j];
			}
		}
			
		else
		{
			for(int j = 0; j < 4; j++)
			{
				round_key[j][i] = round_key[j][i - 4] ^ round_key[j][i - 1];
			}
		}
	}
	
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 44; j++)
		{
			cout << hex << static_cast<int>(round_key[i][j]) << "\t";
		}
		
		cout << endl;
	}
	
	
	return 0;
}
