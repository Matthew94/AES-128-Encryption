...
#include <iostream>												//Importing in libraries
#include <fstream>
#include <string>
...
void open_file(std::ifstream &infile, std::ofstream &outfile)//All of the functions used
{
	...
	std::cout << "Enter the name of a file to be written to: ";		//Prompt to get file name
	std::cin >> file_name;
...
void s_box_k(unsigned char state[4], const unsigned char s[16][16])
{
	...
	for(int i = 0; i < 4; i++)
	{
		unsigned char n = state[i];
		char t = 0xF & n;
		n = n >> 4;
			
		state[i] = s[n][t];										//N then T as it goes rows then columns
	}
}
...
void s_box(unsigned char state[4][4], const unsigned char s[16][16])
{
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 4; j++)
		{
			...
			state[i][j] = s[n][t];										//N then T as it goes rows then columns
		}
	}
}
...
int main()
{
	...
	key_schedule(cipher_key, round_key, a_rot_word, s);							//Expands the entire round key
