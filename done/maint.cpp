...
//Test to see if the program works as expected
...
void open_file			(std::ifstream &infile, 		std::ofstream &outfile)											//Opens input and output text files
{
	...
	std::cout << "Enter the name of a file to be written to: ";															//Prompt to get file name for output of cipher text
	std::cin >> file_name;
	...
	{	
		...
		if(i % 4 == 0)																									//Checks if the current value is a multiple of 4
		{
			...
			if(i == 28)
			{
				for(int o = 0; o < 4; o++)
				{
					std::cout << std::hex << static_cast<int>(a_rot_word[o]) << "\t";
				}
				std::cout << "\n";
			}
			...
			{
				temp[j] = ( round_key[j][i-4] ^ a_rot_word[j] ^ r_con[j][r_con_p] );
				if(i == 28)
				{
					std::cout << std::hex << static_cast<int>(round_key[j][i-4]) << "\t";
					std::cout << std::hex << static_cast<int>(r_con[j][r_con_p]) << "\t\n";
				}
			}
			...
			if(i == 28)
			{
				for(int o = 0; o < 4; o++)
				{
					std::cout << std::hex << static_cast<int>(temp[o]) << "\t";
				}
				std::cout << "\n\n";
			}
			r_con_p++;
...
void rot_word			(unsigned char round_key[4][44],unsigned char a_rot_word[4], int &count)							//Used in key expansion, rotates a column by 1 byte "upwards"
{
	...
	a_rot_word[0] = round_key[1][3+count];																				//Assigns the second value of the round key to the first in the rot_word array
	a_rot_word[1] = round_key[2][3+count];																				//Assigns the third value of the round key to the first in the rot_word array
	a_rot_word[2] = round_key[3][3+count];																				//Assigns the second value of the round key to the first in the rot_word array
	a_rot_word[3] = temp;																									//Assigned the temp variable to the fourth part of the rot_word array
	
	count+=4;																											//Increases count by 4 for the next use of rot_word
...
void encrypt_file()
{
	...
	for(int i = 0; i < 4; i++)
	{
		...
		for(int i = 0; i < 9; i++)																						//Does 9 rounds of the encryption
		{
			...
			std::cout << "S BOX   " << "ROUND: " << (i+1) << "\n";
			for(int k = 0; k < 4; k++)
			{
				for(int j = 0; j < 4; j++)
				{
					std::cout << std::hex << static_cast<int>(state[k][j]) << "\t";
				}
...
int main()
{
	std::cout << std::hex << static_cast<int>((0x6d ^ 0x40 ^ 0x63));
